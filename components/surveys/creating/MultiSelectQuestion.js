...
export default function MultiSelectQuestion({
  ...
  const handleDeleteOption = (indexToDelete) => {
    setQuestionData((prev) => ({
      ...
      correctAnswers: prev.correctAnswers.filter((i) => i !== indexToDelete),
    }));
  };
    ...
    <div className="flex flex-col gap-2 bg-white/50 rounded-2xl p-4">
      <div className="flex flex-row justify-between">
        <div className="flex flex-row items-center gap-2">
          ...
          <p className="text-sm poppins-regular">- Multi Select</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 mt-4">
        {questionData.options.map((option, optIndex) => (
          <div key={optIndex} className="flex items-center gap-2">
            ...
            <input
              type="checkbox"
              checked={questionData.correctAnswers.includes(optIndex)}
        ...
        <div className="flex items-center gap-2 col-span-2">
          <div className="flex-1 flex items-center gap-2">
            <input
              ...
              className="ml-2"
            />
            <span className="text-sm poppins-regular">Other:</span>
            <input
              type="text"
              value={questionData.otherOption}
      ...
      {isEditing && questionData.options.length < 7 && (
        <button
